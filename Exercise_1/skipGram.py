...
class SkipGram:
    ...
    def processing_sentences(self):
        # we start by building a dictionary whose keys are words and values
        # the number of time each word appears in the total corpus
        words = {}
    ...
    def train(self, stepsize=0.1, epochs=7):
        ...
        # parameters used for negative sampling
        alpha = 0.75
        k = self.negativeRate
        L = self.winSize
        
        # using the alpha parameters and words attribute to build
        # a dictionnary giving the probability of a word to be chosen as
        # a negative sample
                ...
                for j in range(len(self.sentences[i])):
                    ...
                    for l in range(max(0, int(j-L/2) + 1), min(len(self.sentences[i]), int(j+L/2) + 2)):
                        if self.sentences[i][l] != target:
                            context_list.append(self.sentences[i][l])
                    
                    # if the target word or a context word is not in the vocabulary,
                    # it is map to a common vector represented by the word '<UNK>'
                    if target not in vocab:
                            target = '<UNK>'
                    for l in range(len(context_list)):
                        if context_list[l] not in vocab:
                            context_list[l] = '<UNK>'
                            
                    for context in context_list:
                        # for each word in the context, we will take negativeRate negative samples
                        # We select randomly words from the vocabulary according to the probability 
                        ...
                        while len(negative) != k:
                            ...
                            if choice != target and choice not in context_list and choice not in negative:
                        ...
                        # if the negative word is not in the vocabulary,
                        # it is map to a common vector represented by the word '<UNK>'
                        for s in range(len(negative)):
                            if negative[s] not in vocab:
                                negative[s] = '<UNK>'
                        ...
                        # defined below
                        similarities_c, similarities_n, t_word_embedding, c_word_embedding, n_emb = \
                                            self.similarities_and_embeddings(target, context, negative)
                        grad_target = self.loss_gradient_target(similarities_c, similarities_n, c_word_embedding, n_emb)
                        ...
                        # we update the target, context and negative samples embeddings using 
                        ...
                        c_word_index = self.word2id[context]
                        self.contexts_emb[c_word_index, :] += -stepsize*grad_context
                        for s in range(len(negative)):
                            n_word_index = self.word2id[negative[s]]
                            self.contexts_emb[n_word_index, :] += -stepsize*grad_negative[s]
                            
    
    def similarities_and_embeddings(self, target, context, negative_list):
        """A function which take the target word, the context word, and the 
        negative words and return the simalirity between the target and the 
        context word (dot product), the similarities between the target word 
        ...
        #getting the context embedding and computing its similarity with the target word
        c_word_index = self.word2id[context]
        c_word_embedding = self.contexts_emb[c_word_index, :]
        similarities_c  = np.dot(c_word_embedding, t_word_embedding)
        
        ##getting the negative embeddings and computing their similarities with the target word
        similarities_n = []
        n_emb = []
        ...
        return similarities_c, similarities_n, t_word_embedding, c_word_embedding, n_emb
        
    
    def loss(self, similarities_c, similarities_n):
        """ A function computing the loss we are trying to minimize"""
        loss = 0
        loss += np.log(expit(similarities_c))
    ...
    def loss_gradient_target(self, similarities_c, similarities_n, c_word_embedding, n_emb):
        """A function computing the gradient of the loss with respect to each
        element of the target embedding"""
        gradient_target = -c_word_embedding*(1-expit(similarities_c))
    ...
    def loss_gradient_context(self, similarities_c, t_word_embedding):
        """A function computing the gradient of the loss with respect to each
        element of the context embedding"""
        gradient_context = -(1-expit(similarities_c))*t_word_embedding
    ...
    def sentence_embeddings(self, tokenized_sentence):
        ...
        for word in tokenized_sentence:
            ...
            sentence_embeddings.append(wi_emb)
        ...
        for j in range(len(sentence_embeddings)):
            sent_emb += sentence_embeddings[j]*idf_values[j]
        sent_emb = sent_emb/len(sentence_embeddings)
